...
public class StringUtils {
	...
	private static final List<String> CURRENCY_CHARS = new ArrayList<String>(14);
...
//					}
	...
	public static String getCurrencySymbol(String val) {
		if (val == null) return "$";
		...
		try {
			...
			for (int i = 0; i < tmpVal.length(); i++) {
				...
				if (CURRENCY_CHARS.contains(tmpChr)) {
	...
	public static double fixDouble(String val) {
		if (val == null) return 0;
		String tmpVal = val.trim();
		if (tmpVal.equals("")) return 0;
		try {
			StringBuffer buff = new StringBuffer();
			...
			for (int i = 0; i < tmpVal.length(); i++) {
				tmpChr = tmpVal.substring(i, i + 1);
				//System.out.println(tmpChr);
				if (tmpChr.equals(".") || NUMERIC_CHARS.contains(tmpChr)) {
					//System.out.println("Adding char = " + tmpChr);
					buff.append(tmpChr);
				}
			}

			if (buff.length() > 0) {
				String pDbl = buff.toString();
				//System.out.println("Final double string = " + pDbl);
	...
	public static long fixLong(String str) throws Throwable {
		...
		if (str == null || str.equals("")) {
			...
			String newStr = StringUtils.replaceAll(str, ",", "");
			newStr = StringUtils.replaceAll(newStr, ".", "");
			retval = Long.parseLong(newStr);
		}
